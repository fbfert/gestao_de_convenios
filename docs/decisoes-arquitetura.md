...
ADR-29 — Credencial de automação pertence ao convênio; `connector_driver` é interruptor, não rótulo de fornecedor
`unimed_rda_credentials` tinha `unique('tenant_id')` e nenhuma coluna de convênio: um tenant tinha no máximo uma credencial, e o convênio correspondente era descoberto procurando `connector_driver = 'unimed_rda'`. Enquanto houve um único convênio automatizado, a diferença entre "a credencial do tenant" e "a credencial daquele convênio" não aparecia.

Ela apareceu em 14/09/2026. Um `WORKER_INTERNAL_FATAL` num único item fez o `UnimedCircuitBreakerService` pausar a credencial do tenant inteiro — `handleResult()` recebia apenas `tenantId` e gravava `ativo = false` na única credencial que existia. A automação de todos os itens parou, e a credencial precisou ser reativada manualmente duas vezes na mesma sessão. Com um segundo convênio automatizado, esse mesmo disjuntor derrubaria também o convênio que não tinha problema nenhum.

A credencial passa a ser identificada por `tenant_id + convenio_id`, e a pausa alcança só o convênio da execução. Os campos de cada fornecedor saem de um catálogo em código (`ConvenioDriverCatalog`), fonte única que a API valida e o front renderiza, então um convênio novo entra sem tela nova — é o ADR-03 aplicado à credencial. O JSON é cifrado inteiro, e não coluna por campo: perde-se a busca por login, que ninguém faz, e ganha-se campo novo de driver futuro sem migration.

A segunda metade do ADR é a que mais custa se for esquecida: **`convenios.connector_driver` continua sendo o interruptor da automação, e o `driver` da credencial é outra coisa.** `connector_driver` tem doze consumidores, entre eles `GuiaService`, `SolicitacaoService`, o job que enfileira consultas ao portal e o `VerificarGuiasDiarioJob` — que **deixa de conferir manualmente** as guias do convênio por assumir que a automação cuida delas. Cadastrar a credencial do SC Saúde com `connector_driver = 'scsaude'` ligaria uma automação que não existe, falharia em toda guia nova e tiraria essas guias da verificação diária, sem ninguém perceber: sumir é o que o job faz de propósito. Por isso `convenio_credenciais.driver` é coluna própria e a tela de credenciais não escreve `connector_driver`.

O change `carteirinha-formato-por-convenio` já havia registrado esse mesmo acoplamento ao desamarrar a máscara de carteirinha do `connector_driver`. Este ADR generaliza a regra.

ADR-30 — Derrubar o escopo global de tenant é privilégio de um arquivo só
`BelongsToTenant` põe um escopo global em todo model de negócio, e é ele que garante que nenhuma consulta do sistema enxergue clínica alheia por esquecimento. A visão "todas as clínicas" dos relatórios, pedida pelo super admin, não tem como sair sem derrubar esse escopo: o `TenantContext` carrega o tenant do usuário autenticado, e é exatamente ele que o escopo aplica.

A decisão é **onde** isso pode acontecer. `withoutGlobalScope(TenantScope::class)` vive apenas em `RelatorioService::naClinica()`, que é o método por onde todo `Builder` dos quatro serviços de relatório passa. Espalhar a chamada pelos serviços transformaria a garantia em convenção — e convenção vaza na primeira consulta que alguém escrever com pressa. Concentrada, ela é revisável por `grep`: o resultado precisa continuar sendo esse arquivo.

Duas consequências assumidas. A primeira: o escopo é derrubado **mesmo quando há uma clínica escolhida**, e o `where('tenant_id', ...)` é reaplicado à mão no mesmo método. Não é descuido. O super admin que escolhe outra clínica tem, no contexto, o tenant dele; manter o escopo global filtraria pela clínica errada e devolveria um relatório vazio com cara de relatório legítimo. Um caminho só, com o filtro sempre explícito, é mais fácil de conferir do que dois caminhos que divergem só no caso raro.

A segunda: a escolha de clínica é barrada em `authorize()` do `RelatorioFiltrosRequest`, com **403 e não 422**. Pedir dado de outra clínica não é campo mal preenchido, é tentativa de acesso — e `authorize()` roda antes da validação, então nem chega a existir mensagem de erro revelando quais ids de tenant existem. O teste de isolamento que acompanha o ADR é obrigatório: usuário comum não vê número de outro tenant nem passando `tenant_id`, e o escopo global continua valendo fora do relatório.

ADR-31 — Histórico de saúde grava a MUDANÇA de estado, e a queda só uma varredura enxerga
`saude_componentes` guarda `ultimo_heartbeat_em` e `ultimo_status`: um retrato do agora, suficiente para o card do painel e inútil para "quanto tempo a fila ficou fora em setembro?". A aba de Automações precisa da segunda pergunta, e daí nasceu `saude_componente_eventos`.

A tabela grava **mudança de estado**, e não cada sinal de vida. O heartbeat é de minuto em minuto: uma linha por heartbeat seriam milhões de registros para responder uma pergunta sobre transições, que só precisa de três ordens de grandeza menos linha.

O ponto que não é óbvio, e que o `tasks.md` da change não previa: **o heartbeat não consegue registrar a queda.** Ele só acontece quando o componente está vivo, então enxerga a volta ao ar e nunca a saída — um worker que morre para de mandar sinal, e ausência de sinal não chama código nenhum. É a passagem do tempo que derruba o estado (`SaudeService::estadoDe()` deriva de quanto faz desde o último heartbeat), e só uma varredura periódica percebe isso. Por isso existe `SaudeService::sincronizarEstados()`, chamado a cada minuto junto do carimbo do agendador em `routes/console.php`. Sem ele, o histórico teria todas as recuperações e nenhuma das quedas, e o relatório responderia "sempre no ar" para um período em que ninguém estava.

O primeiro evento de um componente é sempre gravado, qualquer que seja o estado: é ele que marca onde o histórico começa. Sem essa linha, um componente que nunca caiu não teria evento algum, e o relatório não saberia distinguir "esteve no ar o período todo" de "não tenho registro deste período" — a distinção que a tabela existe para preservar. Consequência aceita: o relatório só enxerga do deploy em diante, e a aba mostra "—" para período anterior, em vez de zero.
