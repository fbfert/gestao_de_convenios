 ...
 * estado para o leitor de tela; o rótulo não fica mais visível no botão (só
 * ícone, a pedido do usuário em 02/09/2026), mas segue disponível via
 * `aria-label`/`title` para quem usa leitor de tela ou passa o mouse.
 */
export function BotaoTema({ className = '' }: { className?: string }) {
  ...
  return (
    <button
      ...
      aria-label={`Tema ${opcaoAtual?.label}. Clique para usar ${opcaoProxima?.label}.`}
      title={`Tema ${opcaoAtual?.label}. Clique para usar ${opcaoProxima?.label}.`}
      data-testid="alternar-tema"
      className={`inline-flex h-10 w-10 items-center justify-center rounded-pilula border border-linha text-texto-suave transition hover:border-acento/40 hover:bg-acento-suave hover:text-acento-intenso ${className}`}
    >
