    ...
    <div className="space-y-8" data-testid="especialidades-page">
      ...
      <section className="space-y-4">
        <div className="flex flex-col gap-4 sm:items-start lg:flex-row lg:items-end lg:justify-between">
          <div>
            <p className="text-meta uppercase tracking-[0.3em] text-cyan-300/80">Especialidades</p>
          ...
          <div className="flex items-center gap-3">
            <Botao type="button" onClick={handleNew} data-testid="especialidade-novo">
              Nova especialidade
            </Botao>
          </div>
        </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4" data-testid="especialidade-form">
            ...
            */}
            <div className="space-y-2" data-testid="especialidade-codigos">
              <span className="text-corpo font-medium text-slate-200">Código por convênio</span>

              {conveniosQuery.isLoading ? (
                <p className="inline-flex min-h-6 items-center text-corpo text-slate-300">Carregando convênios...</p>
              ) : convenios.length === 0 ? (
            ...
            <label className="flex items-center gap-3 rounded-2xl border border-white/10 bg-white/5 px-4 py-3">
              ...
              />
              <span className="text-corpo font-medium text-slate-200">Ativa</span>
            </label>
            ...
            <div className="flex gap-3">
              <Botao
                type="submit"
                className="flex-1"
                carregando={criarEspecialidade.isPending || atualizarEspecialidade.isPending}
      ...
      {isEditRoute && !especialidadeEmEdicao ? (
        ...
        >
          {especialidadesQuery.isLoading ? (
            <p className="inline-flex min-h-6 items-center text-corpo text-slate-300">Carregando especialidade...</p>
          ) : (
      ...
      <section className="space-y-4 rounded-janela border border-linha bg-superficie-elevada shadow-e2 p-6">
        <div className="flex flex-col gap-4 sm:items-start lg:flex-row lg:items-end lg:justify-between">
          ...
          <form className="flex flex-wrap gap-3" onSubmit={handleFilterSubmit}>
            ...
            <Botao type="submit" variante="secundario" data-testid="especialidade-busca-submit">
              Filtrar
            </Botao>
          </form>
        </div>
                ...
                {especialidadesFiltradas.map((especialidade) => (
                  <tr key={especialidade.id} data-testid={`especialidade-row-${especialidade.id}`}>
                    ...
                    <td data-rotulo="Status" className="px-4 py-4">
                      <Badge
                        tone={statusTone(especialidade.ativo)}
