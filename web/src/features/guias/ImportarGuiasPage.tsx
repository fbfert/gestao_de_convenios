...
import {
  baixarTemplateGuias,
  getHttpErrorMessage,
  useConfirmarImportGuias,
  usePrevisualizarImportGuias,
} from './useGuiasImport'
import type { GuiaImportLinha, GuiaImportLinhaDados, GuiaImportPreview } from './types'

const celula =
  'w-full rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-white outline-none focus:border-cyan-300/70'

const STATUS_TONE: Record<GuiaImportLinha['status'], { tone: 'sucesso' | 'perigo' | 'info' | 'neutro'; rotulo: string }> = {
...
const STATUS_OPCOES = [
  ...
  { valor: 'approved', rotulo: 'Autorizado' },
  { valor: 'finalized', rotulo: 'Finalizado' },
  { valor: 'denied', rotulo: 'Negado' },
  { valor: 'canceled', rotulo: 'Cancelado' },
  { valor: 'needs_verification', rotulo: 'Verificar Restrição' },
]

export function ImportarGuiasPage() {
  const navigate = useNavigate()
  const arquivoRef = useRef<HTMLInputElement | null>(null)

  const [preview, setPreview] = useState<GuiaImportPreview | null>(null)
  const [linhas, setLinhas] = useState<GuiaImportLinha[]>([])
  const [selecionadas, setSelecionadas] = useState<Set<number>>(new Set())
  const [edicoes, setEdicoes] = useState<Record<number, Partial<GuiaImportLinhaDados>>>({})
  const [erro, setErro] = useState<string | null>(null)
  const [confirmado, setConfirmado] = useState(false)

  const previsualizar = usePrevisualizarImportGuias()
  const confirmar = useConfirmarImportGuias()
  ...
  const editarCampo = (linhaId: number, campo: keyof GuiaImportLinhaDados, valor: string | number | null) => {
    setLinhas((atual) =>
  ...
  return (
    <div className="space-y-6" data-testid="importar-guias-page">
      <div>
        ...
        <p className="mt-1 text-corpo text-slate-300">
          O número da guia é a chave: reimportar o mesmo número atualiza em vez de duplicar.
        </p>
      </div>

      <section className="space-y-4 rounded-janela border border-linha bg-superficie-elevada shadow-e2 p-6">
        <div className="flex flex-wrap items-center gap-3">
          <Botao variante="secundario" onClick={() => void baixarTemplateGuias()} data-testid="importar-guias-modelo">
            Baixar modelo
          </Botao>
          <Botao
            variante="primario"
            ...
            data-testid="importar-guias-arquivo-botao"
          >
          ...
          <input
            ...
            className="hidden"
            data-testid="importar-guias-arquivo"
          />
        </div>
      ...
      {preview && resumo ? (
        <section className="space-y-3 rounded-janela border border-emerald-400/20 bg-emerald-500/10 p-6 text-emerald-50">
          ...
          <p className="text-corpo">
            {resumo.total_importados} guia(s) nova(s), {resumo.total_atualizados} atualizada(s),{' '}
            {resumo.total_ignorados} ignorada(s)
            {resumo.total_invalidas > 0 ? `, ${resumo.total_invalidas} com erro (não importada)` : ''}.
          </p>
          <Botao variante="primario" onClick={() => navigate('/guias')} data-testid="importar-guias-concluir">
            Voltar para Guias
          </Botao>
        </section>
      ) : null}
      ...
      {preview && !confirmado ? (
        <section className="space-y-4 rounded-janela border border-linha bg-superficie-elevada shadow-e2 p-6">
          <div className="rounded-2xl border border-cyan-400/20 bg-cyan-500/10 px-4 py-3 text-corpo text-cyan-50">
            Nenhuma guia foi salva ainda. Confira e edite as linhas abaixo, marque as que deseja
            importar e confirme.
          </div>
          ...
          <div className="overflow-x-auto rounded-superficie border border-linha">
            <table className="w-full min-w-[100rem] border-collapse text-left text-corpo" data-cartoes="lg">
                ...
                <tr>
                  <th className="px-4 py-3">
                    <input
                      ...
                      data-testid="importar-guias-selecionar-todas"
                    />
                  </th>
                  ...
                  <th className="px-4 py-3">Tipo terapia</th>
              ...
              <tbody className="divide-y divide-linha bg-superficie">
                {linhas.map((linha) => (
                  <tr key={linha.id} data-testid={`importar-guias-linha-${linha.id}`}>
                    <td className="px-4 py-3">
                      <input
                        ...
                        className="size-4 rounded border-white/20 bg-white/10"
                        data-testid={`importar-guias-checkbox-${linha.id}`}
                      />
                    </td>
                    ...
                    <td className="px-4 py-3">
                      <input
                        ...
                        onChange={(event) => editarCampo(linha.id, 'paciente_carteirinha', event.target.value)}
                        className={celula}
                      />
                    </td>
                    <td className="px-4 py-3">
                    ...
                    <td className="px-4 py-3">
                      <input
                        value={linha.dados.tipo_terapia}
                        onChange={(event) => editarCampo(linha.id, 'tipo_terapia', event.target.value)}
                        className={celula}
                      />
                      {linha.erros.tipo_terapia ? (
                        <p className="mt-1 text-meta text-rose-300">{linha.erros.tipo_terapia}</p>
                      ) : null}
                    </td>
                    <td className="px-4 py-3">
                      <input
                        type="date"
                        value={linha.dados.data_solicitacao ?? ''}
                        onChange={(event) => editarCampo(linha.id, 'data_solicitacao', event.target.value)}
                        className={celula}
                      />
                      {linha.erros.data_solicitacao ? (
                        <p className="mt-1 text-meta text-rose-300">{linha.erros.data_solicitacao}</p>
                      ) : null}
                    </td>
                    <td className="px-4 py-3">
                      <Select
                    ...
                    <td className="px-4 py-3">
                      <input
                        value={linha.dados.senha ?? ''}
                        onChange={(event) => editarCampo(linha.id, 'senha', event.target.value)}
                        className={celula}
                      />
                    </td>
                    <td className="px-4 py-3">
                      <Badge tone={STATUS_TONE[linha.status].tone}>{STATUS_TONE[linha.status].rotulo}</Badge>
                      {linha.matched_guia_id ? (
                        <p className="mt-1 text-meta text-slate-400">Atualiza #{linha.matched_guia_id}</p>
                      ) : null}
                    </td>
                  </tr>
                ))}
          ...
          <div className="flex flex-wrap items-center justify-between gap-3">
            ...
            <div className="flex gap-3">
              ...
              <Botao
                ...
                data-testid="importar-guias-confirmar"
              >
