...
import {
  baixarTemplateSolicitacoes,
  getHttpErrorMessage,
  useConfirmarImportSolicitacoes,
  usePrevisualizarImportSolicitacoes,
} from './useSolicitacoesImport'
import type {
  SolicitacaoImportLinha,
  SolicitacaoImportLinhaDados,
  SolicitacaoImportPreview,
} from './types'

const celula =
  'w-full rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-white outline-none focus:border-cyan-300/70'

const STATUS_TONE: Record<SolicitacaoImportLinha['status'], { tone: 'sucesso' | 'perigo' | 'info' | 'neutro'; rotulo: string }> = {
...
const STATUS_OPCOES: { valor: SolicitacaoImportLinhaDados['status']; rotulo: string }[] = [
  { valor: 'under_review', rotulo: 'Em análise' },
  { valor: 'denied', rotulo: 'Negado' },
  { valor: 'canceled', rotulo: 'Cancelado' },
  { valor: 'expired', rotulo: 'Vencido' },
]

export function ImportarSolicitacoesPage() {
  const navigate = useNavigate()
  const arquivoRef = useRef<HTMLInputElement | null>(null)

  const [preview, setPreview] = useState<SolicitacaoImportPreview | null>(null)
  const [linhas, setLinhas] = useState<SolicitacaoImportLinha[]>([])
  const [selecionadas, setSelecionadas] = useState<Set<number>>(new Set())
  const [edicoes, setEdicoes] = useState<Record<number, Partial<SolicitacaoImportLinhaDados>>>({})
  const [erro, setErro] = useState<string | null>(null)
  const [confirmado, setConfirmado] = useState(false)

  const previsualizar = usePrevisualizarImportSolicitacoes()
  const confirmar = useConfirmarImportSolicitacoes()
  ...
  const editarCampo = (
    linhaId: number,
    campo: keyof SolicitacaoImportLinhaDados,
  ...
  return (
    <div className="space-y-6" data-testid="importar-solicitacoes-page">
      <div>
        ...
        <p className="mt-1 text-corpo text-slate-300">
          Cada linha é um item (especialidade + profissional). Linhas com o mesmo Protocolo viram
          itens da mesma solicitação, e reimportar o mesmo Protocolo atualiza em vez de duplicar.
        </p>
      </div>
      ...
      <section className="space-y-4 rounded-janela border border-linha bg-superficie-elevada shadow-e2 p-6">
        <div className="flex flex-wrap items-center gap-3">
          <Botao
            variante="secundario"
            onClick={() => void baixarTemplateSolicitacoes()}
            data-testid="importar-solicitacoes-modelo"
          >
            Baixar modelo
          </Botao>
          ...
          <Botao
            ...
            data-testid="importar-solicitacoes-arquivo-botao"
          >
          ...
          <input
            ...
            className="hidden"
            data-testid="importar-solicitacoes-arquivo"
          />
        </div>
      ...
      {preview && resumo ? (
        <section className="space-y-3 rounded-janela border border-emerald-400/20 bg-emerald-500/10 p-6 text-emerald-50">
          ...
          <p className="text-corpo">
            {resumo.total_importados} item(ns) novo(s), {resumo.total_atualizados} atualizado(s),{' '}
            {resumo.total_ignorados} ignorado(s)
            {resumo.total_invalidas > 0 ? `, ${resumo.total_invalidas} com erro (não importado)` : ''}.
          </p>
          <Botao variante="primario" onClick={() => navigate('/solicitacoes')} data-testid="importar-solicitacoes-concluir">
      ...
      {preview && !confirmado ? (
        <section className="space-y-4 rounded-janela border border-linha bg-superficie-elevada shadow-e2 p-6">
          <div className="rounded-2xl border border-cyan-400/20 bg-cyan-500/10 px-4 py-3 text-corpo text-cyan-50">
            Nenhuma solicitação foi salva ainda. Confira e edite as linhas abaixo, marque as que
            deseja importar e confirme.
          </div>

          <div className="overflow-x-auto rounded-superficie border border-linha">
            <table className="w-full min-w-[100rem] border-collapse text-left text-corpo" data-cartoes="lg">
                ...
                <tr>
                  <th className="px-4 py-3">
                    <input
                      ...
                      data-testid="importar-solicitacoes-selecionar-todas"
                    />
                  </th>
                  <th className="px-4 py-3">Linha</th>
                  <th className="px-4 py-3">Protocolo</th>
                  <th className="px-4 py-3">Paciente CPF</th>
                  <th className="px-4 py-3">Paciente Carteirinha</th>
                  <th className="px-4 py-3">Convênio</th>
                  <th className="px-4 py-3">Médico</th>
                  <th className="px-4 py-3">CID(s)</th>
              ...
              <tbody className="divide-y divide-linha bg-superficie">
                {linhas.map((linha) => (
                  <tr key={linha.id} data-testid={`importar-solicitacoes-linha-${linha.id}`}>
                    <td className="px-4 py-3">
                      <input
                        ...
                        className="size-4 rounded border-white/20 bg-white/10"
                        data-testid={`importar-solicitacoes-checkbox-${linha.id}`}
                      />
                    </td>
                    <td className="px-4 py-3 text-slate-400">{linha.linha}</td>
                    <td className="px-4 py-3">
                      <input
                        value={linha.dados.protocolo ?? ''}
                        onChange={(event) => editarCampo(linha.id, 'protocolo', event.target.value)}
                        className={celula}
                      />
                      {linha.erros.grupo ? (
                        <p className="mt-1 text-meta text-rose-300">{linha.erros.grupo}</p>
                      ) : null}
                    </td>
                    <td className="px-4 py-3">
                      <input
                        value={linha.dados.paciente_cpf ?? ''}
                        onChange={(event) => editarCampo(linha.id, 'paciente_cpf', event.target.value)}
                        className={celula}
                      />
                      {linha.erros.paciente_cpf ? (
                        <p className="mt-1 text-meta text-rose-300">{linha.erros.paciente_cpf}</p>
                      ) : null}
                    </td>
                    <td className="px-4 py-3">
                      <input
                        value={linha.dados.paciente_carteirinha ?? ''}
                        onChange={(event) =>
                          editarCampo(linha.id, 'paciente_carteirinha', event.target.value)
                        }
                        className={celula}
                      />
                    </td>
                    ...
                    <td className="px-4 py-3">
                      <input
                        value={linha.dados.medico}
                        onChange={(event) => editarCampo(linha.id, 'medico', event.target.value)}
                        className={celula}
                      />
                      {linha.erros.medico ? (
                        <p className="mt-1 text-meta text-rose-300">{linha.erros.medico}</p>
                      ) : null}
                    </td>
                    <td className="px-4 py-3">
                      <input
                        value={linha.dados.cids}
                        onChange={(event) => editarCampo(linha.id, 'cids', event.target.value)}
                        className={celula}
                      />
                      {linha.erros.cids ? (
                        <p className="mt-1 text-meta text-rose-300">{linha.erros.cids}</p>
                      ) : null}
                    </td>
                    <td className="px-4 py-3">
                      <input
                        type="date"
                        value={linha.dados.solicitado_em ?? ''}
                        onChange={(event) => editarCampo(linha.id, 'solicitado_em', event.target.value)}
                        className={celula}
                      />
                      {linha.erros.solicitado_em ? (
                        <p className="mt-1 text-meta text-rose-300">{linha.erros.solicitado_em}</p>
                      ) : null}
                    </td>
                    <td className="px-4 py-3">
                      <Select
                        value={linha.dados.status ?? 'under_review'}
                    ...
                    <td className="px-4 py-3">
                      <input
                        value={linha.dados.especialidade}
                        onChange={(event) => editarCampo(linha.id, 'especialidade', event.target.value)}
                        className={celula}
                      />
                      {linha.erros.especialidade ? (
                        <p className="mt-1 text-meta text-rose-300">{linha.erros.especialidade}</p>
                      ) : null}
                    </td>
                    <td className="px-4 py-3">
                      <input
                    ...
                    <td className="px-4 py-3">
                      ...
                      {linha.matched_solicitacao_id ? (
                        <p className="mt-1 text-meta text-slate-400">Atualiza #{linha.matched_solicitacao_id}</p>
                      ) : null}
                    </td>
                  </tr>
                ))}
          ...
          <div className="flex flex-wrap items-center justify-between gap-3">
            ...
            <div className="flex gap-3">
              <Botao variante="secundario" onClick={() => navigate('/solicitacoes')}>
                Cancelar
              </Botao>
              <Botao
                variante="primario"
                ...
                data-testid="importar-solicitacoes-confirmar"
              >
