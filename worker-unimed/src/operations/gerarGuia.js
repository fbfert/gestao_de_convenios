 ...
 * 14/09/2026): limpa o campo, sai dele com Tab (blur de verdade, como um
 * humano faria — a 1a tentativa desse contorno so tinha dado .fill('') sem
 * blur e continuou falhando com a MESMA mensagem) e deixa o Finalizar seguir
 * sem ele. Troca a beneficiaria deixar de receber SMS de status da Unimed
 * por a guia sair do papel — decisao consciente, nao default silencioso
 * (por isso o resultado sinaliza `celular_sms_limpo` quando isso acontece).
 */
async function limparCelularSmsSeInvalido(page) {
  ...
  await campo.click({ timeout: DEFAULT_TIMEOUT }).catch(() => {})
